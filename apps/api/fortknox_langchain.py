...
def run_langchain_llm(prompt: str) -> Tuple[Dict[str, Any], int]:
    """
    Call local OpenAI-compatible endpoint via LangChain ChatOpenAI.
    ...
    model = os.getenv("FORTKNOX_LC_MODEL", "").strip() or "local-model"
    if not base_url:
        raise FortKnoxLangChainConfigError("Missing FORTKNOX_LC_BASE_URL")
